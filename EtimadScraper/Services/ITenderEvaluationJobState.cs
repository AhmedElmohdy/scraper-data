...
public interface ITenderEvaluationJobState
{
    ...
    // Last-run counters
    int LastTotalCandidates { get; }
    int LastTotalEvaluated { get; }
    ...
    void MarkSucceeded(string message, int candidates, int evaluated, int failed);

    /// <summary>Called when a run finishes with an error.</summary>
    void MarkFailed(string error, string message, int candidates, int evaluated, int failed);
}
